...
def _excel_value(header: str, value: str):
    ...
    try:
        if header == "Sr.No.":
            return int(value), "0"
        if header in ("Latitude", "Longitude", "Station Latitude", "Station Longitude", "Speed", "Home Distance (m)", "Linear distance (m)"):
...
def build_rtis_excel(headers: Sequence[str], rows: Iterable[list[str]]) -> bytes:
    ...
    widths = {"Sr.No.": 10, "Device Id": 14, "Loco No.": 14, "Latitude": 14,
              ...
              "Home Signal": 16, "Home Distance (m)": 20, "Linear distance (m)": 20,
              "Station Latitude": 19, "Station Longitude": 20, "Direction": 12,
